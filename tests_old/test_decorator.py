...
from tests.fixtures import decorated_task
...
class TestDecorator(RQTestCase):
    ...
    def test_decorator_accepts_ttl_as_argument(self):
        """Ensure that passing in ttl to the decorator sets the ttl on the task
        """
        # Ensure default
        result = decorated_task.delay(1, 2)
    ...
    def test_decorator_connection_laziness(self, resolve_connection):
        """Ensure that task decorator resolve connection in `lazy` way """
    ...
    def test_decorator_custom_queue_class(self):
        """Ensure that a custom queue class can be passed to the task decorator"""
        ...
        custom_decorator = task(queue='default', queue_class=CustomQueue)
        self.assertIs(custom_decorator.queue_class, CustomQueue)

        @custom_decorator
        def custom_queue_class_task(x, y):
            return x + y

        custom_queue_class_task.delay(1, 2)
        self.assertEqual(CustomQueue.enqueue_call.call_count, 1)

    def test_decorate_custom_queue(self):
        """Ensure that a custom queue instance can be passed to the task decorator"""
        class CustomQueue(Queue):
            pass
        CustomQueue.enqueue_call = mock.MagicMock(
            spec=lambda *args, **kwargs: None,
        ...
        @task(queue=queue)
        def custom_queue_task(x, y):
            return x + y

        custom_queue_task.delay(1, 2)
